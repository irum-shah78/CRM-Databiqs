...
import axios from "axios";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faComments, faTimes, faPaperclip } from "@fortawesome/free-solid-svg-icons";
import Loader from "../loader/Loader";
...
const Chatbot = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFile) {
      setChatHistory((prev) => [
        ...prev,
        { type: "user", text: `Uploaded file: ${selectedFile.name}` },
      ]);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await axios.post(
        "https://chatbot-backend-fastapi-g0fyfwctdehedjay.westus-01.azurewebsites.net/interact",
        formData,
        {
          headers: {
            Accept: "application/json",
